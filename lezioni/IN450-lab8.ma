...
LFSR[a_, b_, k_] := BitOr[LSHIFT[b, 1, k], BitGen[a, b]];
LFSRlength[a_, b_, l_] := Nest[LFSR[a, #, l] &, b, l];
LFSRByte[a_, b_, n_] := LFSRlength[a, b, n 8];
LFSRByteList[a_, b_, n_] := NestList[LFSRByte[a, #, 1] &, b, n];

EncodingFunction[LFSR,k_,m_] := Module[{l},
(
                 l =LFSRByteList[a,k,Length[m]] ;
                 Table[BitXor[m[[i]],l[[i]]] , {i,1,Length[m]}] 
);
...
CodificaFile[cipher_,key_,msgfile_]:=
   Module[{plaintext,ciphercode},
       (
	plaintext = ToCharacterCode[StringJoin@ReadList[msgfile,String]] - 97; Close[msgfile];
	ciphercode = EncodingFunction[cipher,key,plaintext];
        FromCharacterCode[ciphercode + 97]
       )
];
...
cc=ToCharacterCode[c=CodificaFile[SHIFT,7,filename]]-97;
...
(
        ...
        sl = Map[Take[ciphercode, {#, # + k - 1}] & , pl = Flatten[Position[ciphercode, w[[1]]]]];
        gp = Map[ pl[[#]] &, Flatten[Position[sl, w]]];
        Drop[(gp - gp[[1]]), 1]
)]


Map[GCD @@ # &, Table[kasiski[Drop[tamaracc, i], 3], {i, 1, 20}]]
